                  ...
                  <div className="card">
                    ...
                    <div className="cb">
                      {[[calendarEvents.filter((e:any)=>e.block_type==="meet").length,"Meetings"],[calendarEvents.length,"Events"],[calendarEvents.filter((e:any)=>e.meet_link).length,"With Meet"],[calendarEvents.filter((e:any)=>e.block_type==="deep").length,"Focus"]].map(([v,l])=>(
                        <div key={String(l)} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid var(--bdr)",fontSize:12}}><span className="dim">{l}</span><span className="mono acc" style={{fontSize:11}}>{v}</span></div>
                      ))}
                    </div>
                  </div>
                </div>
                            ...
                            {pIssues.length>0&&(
                              <div style={{display:"flex",gap:20,padding:"10px 0",borderBottom:"1px solid var(--bdr)",marginBottom:10}}>
                                {[[pIssues.length,"Issues","var(--acc)"],[blockers,"Blockers",blockers>0?"var(--red)":"var(--grn)"],[bugs,"High Bugs",bugs>0?"var(--red)":"var(--grn)"],[`${completion}%`,"Complete",completion>=60?"var(--grn)":completion>=30?"var(--amb)":"var(--red)"]].map(([v,l,c])=>(
                                  <div key={String(l)} style={{textAlign:"center"}}><div style={{fontFamily:"Syne",fontWeight:800,fontSize:18,color:c}}>{v}</div><div style={{fontSize:10,color:"var(--mut)"}}>{l}</div></div>
                                ))}
                              </div>
                            )}
                          ...
                          <div>
                            ...
                            {siResult.key_metrics&&(
                              <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
                                ...
                                ].map(([l,v,c])=>v!==undefined&&v!==null&&<div key={String(l)}><div style={{fontFamily:"DM Mono",fontSize:10,color:"var(--mut)",marginBottom:2}}>{l}</div><div style={{fontFamily:"Syne",fontWeight:700,fontSize:13,color:c}}>{String(v)}</div></div>)}
                              </div>
                            )}
                          </div>
                ...
                <div className="col">
                  <div className="card">
                    ...
                    <div className="cb"><div className="col" style={{gap:10}}>
                      <div className="form-row"><label className="form-label">Product / Feature Name</label><input className="input" value={prdProductName} onChange={e=>setPrdProductName(e.target.value)} placeholder="e.g. AI Alert Suppression Engine"/></div>
                      ...
                      <div className="form-row"><label className="form-label">Or paste raw text</label><textarea className="input" value={prdInput} onChange={e=>setPrdInput(e.target.value)} placeholder="Paste focus group transcript, survey results, or feature brief..." style={{minHeight:120,resize:"vertical"}}/></div>
                      <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{["Web search","OKR alignment","Competitive context","RICE scores","Saves to knowledge base"].map(l=>(<span key={String(l)} style={{fontFamily:"DM Mono",fontSize:9,padding:"3px 8px",borderRadius:100,background:"rgba(124,58,237,0.07)",border:"1px solid rgba(124,58,237,0.15)",color:"var(--pur)"}}>{l}</span>))}</div>
            ...
            {page==="stakeholders"&&(
              <div className="col">
                <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                  {shProjects.map(p=><button key={String(p)} onClick={()=>setShFilter(String(p))} style={{fontFamily:"DM Mono",fontSize:10,padding:"4px 12px",borderRadius:100,cursor:"pointer",border:`1px solid ${shFilter===p?"var(--acc)":"var(--bdr)"}`,background:shFilter===p?"rgba(0,212,255,0.07)":"var(--surf)",color:shFilter===p?"var(--acc)":"var(--mut)"}}>{p}</button>)}
