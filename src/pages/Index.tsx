...
const S = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --bg:#07090f;--surf:#0c1018;--surf2:#101520;--bdr:#192030;--bdr2:#1e2a3d;
    --acc:#00d4ff;--pur:#7c3aed;--grn:#10b981;--amb:#f59e0b;--red:#ef4444;
    --txt:#dde4ef;--mut:#4e5f74;--r:10px;--r-sm:7px;--gap:14px;
  }
  ...
  @media (max-width:800px){
    .sb{width:52px;}.sb-item span:not(.sb-item-ic){display:none;}
    .sb-grp-lbl,.sb-logo-name,.sb-context{display:none;}
    .main{margin-left:52px;}.g2,.g3,.g4{grid-template-columns:1fr;}
    .pb{padding:14px;}.form-grid{grid-template-columns:1fr;}
  }
`;
...
const SCHEDULE: { t: string; type: string; title: string; desc: string; dur: string; ai: string | null }[] = [
...
const EV_CLS: Record<string,string> = { deep:"ev-deep",meet:"ev-meet",admin:"ev-admin",buf:"ev-buf",break:"ev-break" };
...
export default function PMDashboard() {
  ...
  const [privTogs, setPrivTogs] = useState<Record<string,boolean>>({ persist:true, learn:true, session:false, audit:true });
  ...
  useEffect(() => {
    const ch = supabase.channel("tasks-live")
      .on("postgres_changes" as any, { event: "INSERT", schema: "public", table: "tasks" }, (payload: any) => setTodos(p => [payload.new, ...p]))
      .on("postgres_changes" as any, { event: "UPDATE", schema: "public", table: "tasks" }, (payload: any) => setTodos(p => p.map(t => t.id === payload.new.id ? payload.new : t)))
      .on("postgres_changes" as any, { event: "DELETE", schema: "public", table: "tasks" }, (payload: any) => setTodos(p => p.filter(t => t.id !== payload.old.id)))
      .subscribe();
  ...
  useEffect(() => {
    const ch = supabase.channel("projects-live")
      .on("postgres_changes" as any, { event: "*", schema: "public", table: "projects" }, () => loadProjects())
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [loadProjects]);
  ...
  const loadMeetings = useCallback(async () => {
    setMeetLoading(true);
    const { data } = await (supabase.from as any)("meetings").select("*").order("meeting_time", { ascending: false });
  ...
  const loadOkrs = useCallback(async () => {
    setOkrsLoading(true);
    const { data: okrData } = await (supabase.from as any)("okrs").select("*").order("sort_order");
    if (okrData) {
      const withKRs = await Promise.all((okrData as any[]).map(async (obj: any) => {
        const { data: krs } = await (supabase.from as any)("key_results").select("*").eq("okr_id", obj.id).order("sort_order");
        return { ...obj, krs: (krs as any[]) || [] };
      }));
      setOkrs(withKRs);
  ...
  const loadStakeholders = useCallback(async () => {
    setShLoading(true);
    const { data } = await (supabase.from as any)("stakeholders").select("*, stakeholder_projects(project_id, projects(name))").order("name");
    if (data) setStakeholders((data as any[]).map((s: any) => ({
      ...s,
  ...
  const loadMemory = useCallback(async () => {
    const { data } = await (supabase.from as any)("memory_log").select("*").order("created_at", { ascending: false });
  ...
  const openEditProj = (p: any) => { setProjForm({ name: p.name, owner: p.owner||"", status: p.status, progress: String(p.progress), due_date: p.due_date||"" }); setEditProj(p); setShowAddProj(true); };
  const saveProject = async () => {
    ...
    if (editProj) await supabase.from("projects").update(payload).eq("id", editProj.id);
    else await supabase.from("projects").insert({ ...payload, user_id: user?.id });
  ...
  const saveMeeting = async () => {
    ...
    const { data } = await (supabase.from as any)("meetings").insert({ title: meetForm.title.trim(), meeting_time: meetForm.meeting_time || new Date().toISOString(), raw_transcript: meetForm.raw_transcript, user_id: user?.id }).select().single();
    if (data && meetForm.raw_transcript.trim()) {
      try {
        const { data: result } = await supabase.functions.invoke("meeting-scribe", { body: { transcript: meetForm.raw_transcript, title: meetForm.title, meetingId: (data as any).id } });
        if (result) await loadMeetings();
      } catch {}
    }
    setMeetProcessing(false); setShowAddMeet(false);
    setMeetForm({ title:"", meeting_time:"", raw_transcript:"" });
    loadMeetings();
  };
  const deleteMeeting = async (id: string) => { await (supabase.from as any)("meetings").delete().eq("id", id); loadMeetings(); };
  ...
  const saveKR = async (kr: any) => {
    ...
    await (supabase.from as any)("key_results").update({ current_val: newVal }).eq("id", kr.id);
    setEditingKR(null); loadOkrs();
  };
  ...
  const saveSh = async () => {
    ...
    const payload = { name: shForm.name.trim(), role: shForm.role, email: shForm.email, type: shForm.type, influence: shForm.influence, color: shForm.color, initials: initials(shForm.name) };
  ...
  const forgetMem = async (id: string) => { setMemLog(m => m.filter(x => x.id !== id)); await (supabase.from as any)("memory_log").delete().eq("id", id); };
  const forgetAll = async () => { setMemLog([]); if (user?.id) await (supabase.from as any)("memory_log").delete().eq("user_id", user.id); };
  const togglePriv = async (key: string) => {
    const next = { ...privTogs, [key]: !privTogs[key] };
    setPrivTogs(next);
    if (user?.id) await supabase.from("user_settings").upsert({ user_id: user.id, persistent_memory: next.persist, agent_learning: next.learn, session_only: next.session, audit_log: next.audit });
  };
  ...
  const runPRD = async () => {
    ...
    try {
      ...
      setPrdStatus("idle");
      alert("PRD Agent failed. Check your edge function and API key.");
    }
  };

  const shProjects = ["All", ...Array.from(new Set(stakeholders.flatMap(s => s.proj)))] as string[];
      ...
      <div className="app">
          ...
          <div className="pb">
              ...
              <div className="g2" style={{gridTemplateColumns:"1fr 240px"}}>
                ...
                <div className="col">
                  ...
                  <div className="card">
                    ...
                    <div className="cb">
                      {[["Total tasks", todos.length],["Open", todos.filter(t => t.status==="open").length],["Done", todos.filter(t => t.status==="done").length],["High priority",todos.filter(t => t.priority==="high" && t.status==="open").length]].map(([l,v]) => (
            ...
            {page === "meetings" && (
              <div className="g2">
                <div className="col">
                  <div className="card">
                    <div className="ch"><div className="ct">Meetings</div><button className="btn btn-sm" onClick={() => setShowAddMeet(true)}>+ New Meeting</button></div>
                    <div className="cb0">
                      ...
                      {meetings.map((m,i) => (
                        <div key={m.id} style={{padding:"14px 16px",borderBottom:i<meetings.length-1?"1px solid var(--bdr)":"none"}}>
                          ...
                          {m.action_items && m.action_items.length > 0 && (<><div className="section-lbl" style={{marginBottom:6}}>Action Items</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{m.action_items.map((a: any, j: number) => (<span key={j} className="tag tag-blu">{typeof a === "string" ? a : a.text}</span>))}</div></>)}
                  ...
                  <div className="card">
                    <div className="ch">
                      ...
                      <button className="btn btn-sm" onClick={async () => {
                        ...
                        try { const { data } = await supabase.functions.invoke("prioritization", { body: { items } }); alert(data?.summary || "Scoring complete."); } catch { alert("Prioritization agent not available."); }
            ...
            {page === "okr" && (
              <div className="col">
                <div className="g4">
                  {[
                    {v:`${okrs.length > 0 ? Math.round(okrs.reduce((s,o)=>s+(o.overall_pct||0),0)/okrs.length) : 0}%`,l:"Overall Q2 Progress",c:"var(--acc)"},
                    {v:`${okrs.flatMap(o=>o.krs||[]).filter((kr:any)=>kr.status==="on-track").length}/${okrs.flatMap(o=>o.krs||[]).length}`,l:"KRs On Track",c:"var(--grn)"},
                    {v:`${okrs.flatMap(o=>o.krs||[]).filter((kr:any)=>kr.status==="at-risk").length}/${okrs.flatMap(o=>o.krs||[]).length}`,l:"KRs At Risk",c:"var(--amb)"},
                ...
                {okrs.length === 0 && !okrsLoading && (<div className="empty">No OKRs found. Add them via the database or seed SQL.</div>)}
                ...
                <div className="card">
                  ...
                  <div className="cb">
                    <div className="g3" style={{gap:8}}>
                      {[["LLM","Claude API (Sonnet)"],["Orchestration","Edge Functions"],["Database","Postgres"],["Auth","Lovable Cloud Auth"],["Storage","Cloud Storage"],["Realtime","Realtime Channels"]].map(([l,v]) => (
                    ...
                    <div className="cb">
                      {prdStatus !== "done" ? (
                        ...
                        <div>
                          ...
                          {prdResult.themes.map((t: any, i: number) => (
                    ...
                    <div className="card">
                      ...
                      <div>
                        ...
                        <div className="prd-sec"><div className="prd-lbl">Success Metrics</div>
                          {prdResult.metrics.map((m: string, i: number) => (
                        ...
                        <div className="prd-sec"><div className="prd-lbl">Open Questions</div>
                          {prdResult.questions.map((q: string, i: number) => (
              ...
              <div className="col">
                <div style={{padding:"12px 16px",background:"rgba(16,185,129,0.05)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:10,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                  ...
                  <div style={{display:"flex",gap:7}}>
                    {["Local Postgres","Lovable Cloud","MacBook Pro"].map(l => (
                ...
                <div className="g2">
                  <div className="card">
                    <div className="ch"><div className="ct">Agent Memory Log</div><button className="btn btn-danger btn-sm" onClick={forgetAll}>Forget All</button></div>
                    <div className="cb">
                      {memLog.length===0 ? <div className="empty">Memory cleared</div> : memLog.map(m => (
                  ...
                  <div className="col">
                    ...
                    <div className="card">
                      ...
                      <div className="cb">
                        <p style={{fontSize:12,color:"var(--mut)",lineHeight:1.7,marginBottom:12}}>All data is stored in your Cloud database. Export or permanently delete at any time.</p>
                        <div className="col" style={{gap:6}}>
        ...
        {showAddMeet && (
          <Modal title="New Meeting" onClose={() => setShowAddMeet(false)}>
            <div className="form-row"><label className="form-label">Meeting Title</label><input className="input" value={meetForm.title} onChange={e => setMeetForm(p=>({...p,title:e.target.value}))} placeholder="Sprint Planning — Mobile Team" autoFocus/></div>
            <div className="form-row"><label className="form-label">Date & Time</label><input className="input" type="datetime-local" value={meetForm.meeting_time} onChange={e => setMeetForm(p=>({...p,meeting_time:e.target.value}))}/></div>
            <div className="form-row"><label className="form-label">Transcript (paste for AI processing)</label>
              <textarea className="input" value={meetForm.raw_transcript} onChange={e => setMeetForm(p=>({...p,raw_transcript:e.target.value}))}
                placeholder="Paste meeting transcript here..." style={{minHeight:140,resize:"vertical"}}/>
            </div>
            <div className="form-actions">
